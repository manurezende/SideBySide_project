  ...
  fetch("http://localhost:3000/jovem/cadastrar", {
    ...
    .then(rs => {
      if (rs.erro) {
        ...
        alert(rs.msg);
        console.log("Resposta completa do backend:", rs); // Isso já mostra o payload no console

        // --- NOVO CÓDIGO AQUI: SALVAR DADOS DO JOVEM NO LOCALSTORAGE ---
        ...
        if (jovemCadastrado) {
          // Criar um objeto simplificado com as informações que você precisa
          const dadosParaSalvar = {
            id_jovem: jovemCadastrado.id_jovem,
            id_usuario: jovemCadastrado.id_usuario,
            nome_completo: jovemCadastrado.nome_completo, // Se o backend retornar o nome completo também
            foto_perfil_url: jovemCadastrado.foto_jovem, // A URL/caminho da foto
            tipo_usuario: 1 // 1 para jovem, como definido no seu cadastro
          };

          // Salvar no localStorage. Converte o objeto para string JSON.
          localStorage.setItem('usuarioLogado', JSON.stringify(dadosParaSalvar));
          console.log("Dados do jovem salvo no localStorage após cadastro:", dadosParaSalvar);

          // Agora você pode usar essa informação para atualizar a foto do avatar
          const imgPerfilAvatar = document.querySelector('header .avatar'); // Seleciona o avatar do header
          if (imgPerfilAvatar) {
            const fotoUrlCompleta = `http://localhost:3000${dadosParaSalvar.foto_perfil_url}`;
            imgPerfilAvatar.src = fotoUrlCompleta;
            console.log("Foto de perfil do avatar atualizada para:", fotoUrlCompleta);
          } else {
            console.warn("Elemento avatar não encontrado no header.");
          }
        } else {
          console.warn("Não foi possível encontrar os dados do jovem recém-cadastrado na resposta do backend.");
        }
...
async function cadastrar_idoso() {
  ...
  try {
    ...
    if (mensagemSucessoBackend === "idoso cadastrado com sucesso!") {
      ...
      if (idosoCadastrado) {
        // É crucial que o backend retorne o 'nome_completo' do usuário associado no payload do idoso
        // ou que você o obtenha de outra forma se ele for necessário para o localStorage.
        const usuarioTempJSON = localStorage.getItem('usuarioLogadoTemp');
        let nomeCompleto = 'Nome Desconhecido';
        if (usuarioTempJSON) {
          const usuarioTemp = JSON.parse(usuarioTempJSON);
          nomeCompleto = usuarioTemp.nome_completo || nomeCompleto;
          localStorage.removeItem('usuarioLogadoTemp'); // Remover depois de usar
        }

        const dadosParaSalvar = {
          id_idoso: idosoCadastrado.id_idoso,
          id_usuario: idosoCadastrado.id_usuario,
          nome_completo: nomeCompleto, // Usa o nome completo obtido ou o padrão
          foto_perfil_url: idosoCadastrado.foto_idoso, // A URL/caminho da foto do idoso
          tipo_usuario: 0 // 0 para idoso
        };

        localStorage.setItem('usuarioLogado', JSON.stringify(dadosParaSalvar));
        console.log("Dados do idoso salvo no localStorage após cadastro:", dadosParaSalvar);

        const avatarImg = document.querySelector('header .avatar');
        if (avatarImg && dadosParaSalvar.foto_perfil_url) {
          const fotoUrlCompleta = `http://localhost:3000${dadosParaSalvar.foto_perfil_url}`;
          avatarImg.src = fotoUrlCompleta;
          console.log("Foto de perfil do avatar (header) atualizada para idoso:", fotoUrlCompleta);
        }
      } else {
        console.warn("Não foi possível encontrar os dados do idoso recém-cadastrado na resposta do backend.");
      }
      // --- FIM DO CÓDIGO ADICIONADO/MODIFICADO ---
